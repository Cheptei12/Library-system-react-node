...
const VendorManagement = () => {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        // Ensure specialties is an array for each vendor
        const parsedVendors = response.data.map(vendor => ({
          ...vendor,
          specialties: typeof vendor.specialties === "string" ? JSON.parse(vendor.specialties) : vendor.specialties || [],
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/vendors", {
        ...
        specialties: JSON.stringify(newVendor.specialties), // Send as JSON string
      });
      setVendors([...vendors, { ...newVendor, vendor_id: response.data.vendorId, contact_person: newVendor.contactPerson, specialties: newVendor.specialties }]);
      setNewVendor({
        name: "",
        contactPerson: "",
        email: "",
  ...
  const handleEdit = (vendor) => {
    setEditVendor({
      ...vendor,
      specialties: Array.isArray(vendor.specialties) ? vendor.specialties : JSON.parse(vendor.specialties || "[]"),
    });
    setShowModal(true);
  };
  ...
  const handleModalSave = async () => {
    try {
      await axios.put(`http://localhost:5000/api/vendors/${editVendor.vendor_id}`, {
        ...
        specialties: JSON.stringify(editVendor.specialties), // Send as JSON string
      });
      setVendors(vendors.map((vendor) => 
        vendor.vendor_id === editVendor.vendor_id 
          ? { ...editVendor, specialties: editVendor.specialties } 
          : vendor
      ));
      setShowModal(false);
    } catch (error) {
        ...
        <tbody>
          {vendors.map((vendor) => (
            <tr key={vendor.vendor_id}>
              ...
              <td>{vendor.email}</td>
              <td>{vendor.phone}</td>
              <td>{Array.isArray(vendor.specialties) ? vendor.specialties.join(", ") : vendor.specialties || "None"}</td>
              <td>
                <Button
                  variant="warning"
                  size="sm"
